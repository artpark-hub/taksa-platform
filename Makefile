...
SERVICES := device-management user-management ui-service

.DEFAULT_GOAL := help

.PHONY: help build build-dm build-user build-ui clean

help: ## Show this help message
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-20s\033[0m %s\n", $$1, $$2}'

build: build-dm build-user build-ui ## Build all platform service Docker images
