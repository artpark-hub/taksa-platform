export const defaultBufferYaml = 'buffer:\n  none: {}\n';

export const normalizeProtocolKey = (protocol) => String(protocol || '')
    .trim()
    .toLowerCase()
    .replace(/[\s_-]/g, '');

export const ensureTrailingNewline = (value, fallback = '') => {
...
export const addTemplateVariables = (target, source) => {
    if (!source) {
        return;
    }
...
export const templateVariablesToArray = (variables) => {
...
export const normalizeLocationLevels = (location) => {
...
export const normalizeFacadeResult = (result) => result?.result || result?.data?.result || result?.data || result || {};

export const buildProtocolFacadeRequest = ({
    bridgeConfig,
    deviceId,
    uuid,
    location,
...
export const facadeResultToBridgeConfig = ({
    result,
    selectedDeviceName = '',
    fallbackBridgeId = '',
    protocol,
