...
// DeviceCertificateStore implements storage.DeviceCertificateStore for SQLite
...
func (s *DeviceCertificateStore) UpdateDevice(ctx context.Context, deviceID string, certificate *v2.Certificate) error {
	...
	result, err := s.db.ExecContext(ctx,
		`UPDATE device_certificates SET certificate = ? WHERE device_id = ?`,
		certificate.Certificate,
		deviceID)
...
func (s *DeviceCertificateStore) DeleteByDevice(ctx context.Context, deviceID string) error {
	...
	_, err := s.db.ExecContext(ctx, "DELETE FROM device_certificates WHERE device_id = ?", deviceID)
	if err != nil {
...
func (s *DeviceCertificateStore) CleanupExpired(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx,
		"DELETE FROM device_certificates WHERE expires_at < datetime('now') AND expires_at IS NOT NULL")
	if err != nil {
