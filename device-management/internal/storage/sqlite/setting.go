...
// SettingStore implements storage.SettingStore for SQLite
...
func (s *SettingStore) SetWithDescription(ctx context.Context, key, value, description string) error {
	...
	_, err := s.db.ExecContext(ctx, query, key, value, description, value, description)
	if err != nil {
...
func (s *SettingStore) Exists(ctx context.Context, key string) (bool, error) {
	...
	var exists bool
	err := s.db.QueryRowContext(ctx,
		"SELECT EXISTS(SELECT 1 FROM settings WHERE key = ?)", key).Scan(&exists)
